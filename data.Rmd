...
Packages needed for the data preparatin are loaded. If the libraries do not exist locally, they will be downloaded.
...
leaflet() %>%
  ...
  setView(-8,53.5,6)%>%
  addMarkers(lng = -6.9121167, lat = 52.8560667,
             label = "Oak Park",
             # color = "green",
             icon=greenLeafIcon
              # radius = 5
...
hist(OP$temp,col = "peachpuff", main = "Histogram of Temperatures",prob = TRUE) # prob = TRUE to show densities instead of frequencies
...
hist(OP$rhum,col = "lightblue", main = "Histogram of Temperatures",prob = TRUE) 
...
legend(x = "topleft", 
       ...
       col = c("chocolate3", "royalblue", "red", "darkgreen","green"),
       lwd = c(2, 2, 2))
hist(OP$rain[OP$rain>0], breaks = 15, main = "Freq. of rain > 0 mm",
     xlab = "Rain (mm)/hour")
```
We can see that there is no suspicious vlaues in the data.  Rain is often ocurenc at Oak Park, although those are not heavy rains, whihc makes it a very good ground for the spread of crop pathogens. 
...
```{r}
 OP %>%
  ...
  kable(caption = "Night-time Averages 8PM:6AM")
```
...
OP %>% 
  ...
  geom_vline(xintercept=seq(0,24,6), size = 0.1, color = "gray", linetype="dotted")+
  ...
  labs(title= "Average temperatures per hour for months April to September",
       y = "Temperature",
       x = "Hours of the day",
       color = "10C Line")
```
...
ggplot(aes(month_abb,rain,fill = factor(year_var)))+
  ...
  annotate("text",x = 1:6, y = 51, 
          label=as.character(OP_months$rain))+
  theme_minimal()


```
```{r wth_daily, fig.height=18, fig.width=11}
OP %>%
    filter(month %in% c(4:10)) %>%
    group_by(year_var, month, short_date) %>% 
    summarize(temp = round(mean(temp),1),
