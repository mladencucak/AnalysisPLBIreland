...
Histogram of variables used in the analysis
...
```
```{r wth_daily, fig.height=13, fig.width=11}
OP %>%
    filter(month %in% c(4:10)) %>%
    group_by(year_var, month, short_date) %>% 
    summarize(temp = round(mean(temp),1),
  ...
  ggplot() +
    ...
    geom_col(
      aes(date,
          rain,
          colour = "Precipitation (mm/h)",
          fill = "Precipitation (mm/h)"),
      size = 1 ,
      inherit.aes = TRUE,
      width = 0.05
    ) +
