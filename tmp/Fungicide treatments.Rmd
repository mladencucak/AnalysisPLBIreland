...
```
#The Rationale  
The cop risk predicion model is usefull only if it provides provides same level of prottection as the standard practice, while reducing neccessary costs and labour. It was evident that the Irish Rules with their default variable thresholds were to conservative and failed to prove as sustainable solution for the disease forecasting. Two alternative model paramtarisations are proposed, it is neccessary to evaluate if there is a  need for 
# Cut-off dates
