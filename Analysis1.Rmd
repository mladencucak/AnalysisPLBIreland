...
PlotROC <- function(df, numbering = NULL) {
  ...
  if("model"%in% names(AUROC_data)){
    ...
    if(str_split(unique(df$model), "_")[[1]][4] == "90"){
  ...
  if("model"%in% names(AUROC_data)){
   title <- paste("Model parameters =", str_replace_all(df[1, 1],"_", ";"))
  } else{
      pars <- str_split(df[1,1], "_")
    title <- 
      paste0( letters[numbering], ") ",
        "ROC for ", 
       "Tt = ", pars[[1]][[2]],"°C;",
       "RHt = ", pars[[1]][1], "%; ",
        "\n",
       "SDt = ", pars[[1]][3], " hours; and LW = rain and RH>90%.")
    }
...
prettyModel <-  gsub("-", "- ", prettyModel)
cat(paste("AUROC =",prettyModel));rm(processPolyNames,coefs,prettyNames)
...
Plots show relationship between variation in parameters of two variables with third variable fixed at three levels and response as AUROC. 3D surface plots indicate that reduction teperature and relative humidity threshold improves accuracy of the model; while teperature threshold should be increased to acheve same effect. 
...
```{r table_of_models}
# parameters[parameters[["model"]] %in% c("90_10_12_rain",
#                                    "88_10_10_r+rh",
#                                    "87_13_12_r+rh",
#                                    "88_12_9_r+rh" 
#                                    ),] %>% 
...
```{r further_analysis, fig.show = "hold",fig.width=5, fig.height=5, warning=FALSE, warning=FALSE,fig.path='figures/', dev=c('png', 'pdf')}
mod_list <-  list(ROC_data[["90_10_12_rain"]],
...
map2(mod_list, c(1:length(mod_list)), PlotROC)
```
...
Theoretical investigation of number of treatments recomended by selected models was run
