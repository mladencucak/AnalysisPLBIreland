...
PlotROC <- function(df, numbering = NULL) {
  ...
  pars <- str_split(df[1,1], "_")
    title <- 
      paste0( ifelse(is.null(numbering),"",  paste(letters[numbering],") ")), 
              "ROC for ", 
              "Tt = ", pars[[1]][[2]],"°C;",
              "RHt = ", pars[[1]][1], "%; ",
              "SDt = ", pars[[1]][3], 
              "\n",
              " hours; and LW = rain and RH>90%.")
...
```{r further_analysis, fig.show = "hold", warning=FALSE, fig.width=13, fig.height=18,message=FALSE, fig.path='figures/', dev=c('png', 'pdf')}
