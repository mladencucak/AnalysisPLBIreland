...
```{r desc_stat, fig.align='center',fig.show = "hold"}
p1 <-  
...
Plots show relationship between variation in parameters of two variables with third variable fixed at three levels and response as AUROC. 

### Further investigation

Based on the sensitivity analysis followig model parametrisations were compared to the original parameters of Itish ruels and further analised: 

```{r}
parameters %>% 
  filter(model %in% c("90_10_12_rain","88_10_10_r+rh","87_13_12_r+rh","88_12_9_r+rh" )) %>% 
  select(-"model") %>% 
  gt()
```

```{r further_analysis, out.width = '50%',fig.show = "hold", warning=FALSE}
PlotROC(ROC_data[["90_10_12_rain"]])
...
myimages <-
  list.files(here("data", "op_2007_16", "graph", "yearly_plots"),
    pattern = ".png",
    full.names = TRUE
  )
