...
```{r include=FALSE}
p1 <- PlotROC( ROC_data[["90_10_12_rain"]])
p2 <- PlotROC( ROC_data[["88_10_10_90"]])
p3 <- PlotROC( ROC_data[["87_13_12_90"]])
p4 <- PlotROC( ROC_data[["88_12_9_90"]])

png(filename = paste("./data/op_2007_16/results/ROC_plots.png"), width = 800, height = 800,bg = "white", res = NA, restoreConsole = TRUE)
...
```{r fig.align='center', out.width = '80%',out.height = '100%', warning=FALSE}
...
p2 <- ggplot(data, aes(factor(rh_thresh), auc))+
  geom_boxplot(width= 0.4)+
    geom_jitter(position = position_jitter(width = 0.2), colour= "black", alpha = 0.6,size = 0.7)+
...
p3 <-  ggplot(data, aes(factor(temp_thres), auc))+
  geom_boxplot(width= 0.4)+
    geom_jitter(position = position_jitter(width = 0.2), colour= "black", alpha = 0.6,size = 0.7)+
...
### Model fitting 
Thresholds evaluated in this study. 
```{r}
...
rpoly_1_fit <-  lm(auc ~ poly(rh, h, t, degree = 1), data = cd_data)
rpoly_2_fit <-  lm(auc ~ poly(rh, h, t, degree = 2), data = cd_data)
rpoly_3_fit <-  lm(auc ~ poly(rh, h, t, degree = 3), data = cd_data)
rpoly_4_fit <-  lm(auc ~ poly(rh, h, t, degree = 4), data = cd_data)

mod_ls <- list(rpoly_1_fit, rpoly_2_fit, rpoly_3_fit, rpoly_4_fit)
mod_ls <- lapply(mod_ls, function(x) x$studres <- rstudent(x))

compareLM(rpoly_1_fit,rpoly_2_fit,rpoly_3_fit,rpoly_4_fit)[[2]] %>% gt() 

anova(rpoly_1_fit, rpoly_2_fit, rpoly_3_fit, rpoly_4_fit)
...
abline(h = 0, col = "red")
hist(resid(rsm_fit)) #distriburion of residuals is approximately  normal
```

Extract the model formula. the code has been borrowd from [this stackoverflow thread]([I'm an inline-style link](https://www.google.com))
```{r}
