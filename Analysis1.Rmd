...
```{r message=FALSE, warning=FALSE, fig.dim = c(6, 4)}
...
###
With ROC curves constructed AUC is calculated for each model variation. 
```{r}
...
AUC_data <- lapply(ROC_data, function(x) GetAUC(x));rm(ROC_data)
AUC_data <- lapply(AUC_data, function(x) mutate_if(x,is.factor, as.character))
AUC_data <- bind_rows(AUC_data)
```

Each model variation is now transformed from character string to a factor vector. 
```{r}
...
AUC_data[,parameters] <- lapply(AUC_data[,parameters], as.numeric); rm(parameters)
data<- data.frame(AUC_data)
head(data) %>% gt()
```
...
We shall consult visual aids to assess if distribution of differences follows normal distribution.
...
Use paired-sample Wilcoxon test to determine if median auc with r as lw estimator is as good as the median auc with both r and rh as estimators. 

```{r}
w.test <- with(t_data, wilcox.test(r, r_rh, paired = TRUE,
                                   exact = F,
                                   alternative = "less"))
w.test
```
...
```
Increasing temperature threshold had positive, while reducing rh and duration of sporulation has negative with increase in AUROC.
...
data[order(data$hours),] %>% 
  ...
  ggtitle("Boxplot of different durations of minimum sporulation period")+
  geom_smooth(method = "lm", se=T, color="red", aes(group=1))+
  xlab("Duration of sporulation period")+
  theme_bw()
```
...
ggplot(data, aes(factor(rh_thresh), auc))+
  geom_boxplot(width= 0.3)+
  ggtitle("Boxplot of different rh thresholds for sporulation and infection")+
  geom_smooth(method = "lm", se=T, color="blue", aes(group=1))+
  theme_tufte()
```
...
ggplot(data, aes(factor(temp_thres), auc))+
  geom_boxplot(width= 0.3)+
  ggtitle("Boxplot of different temperature thresholds for sporulation and infection")+
  geom_smooth(method = "lm", se=T, color="black", aes(group=1), show.legend = T)+
  theme_tufte()
```
...
persp(rsm_fit, ~ h + t,zlab = "AUC",at = data.frame(t = 0, rh = -3, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 50, phi = 10,  shade  = .2)
persp(rsm_fit, ~ h + t,zlab = "AUC",at = data.frame(t = 0, rh = 0, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 50, phi = 10,  shade  = .2)
persp(rsm_fit, ~ h + t,zlab = "AUC", at = data.frame(t = 0, rh = +3, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 50, phi = 10,  shade  = .2)

persp(rsm_fit, ~ rh + t,zlab = "AUC", at = data.frame(t = 0, rh = 0, h = -3 ), col = color, zlim = c(z_min,z_max),theta = 60, phi = 10,  shade  = .2)
persp(rsm_fit, ~ rh + t,zlab = "AUC", at = data.frame(t = 0, rh = 0, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 50, phi = 10,  shade  = .2)
persp(rsm_fit, ~ rh + t,zlab = "AUC", at = data.frame(t = 0, rh = 0, h = +3 ), col = color, zlim = c(z_min,z_max),theta = 50, phi = 10,  shade  = .2)

persp(rsm_fit, ~ h + rh,zlab = "AUC", at = data.frame(t = -3, rh = 0, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 60, phi = 10,  shade  = .2)
persp(rsm_fit, ~ h + rh,zlab = "AUC", at = data.frame(t = 0, rh = 0, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 60, phi = 10,  shade  = .2)
persp(rsm_fit, ~ h + rh,zlab = "AUC", at = data.frame(t = +3, rh = 0, h = 0 ), col = color, zlim = c(z_min,z_max),theta = 60, phi = 10, r = 1.9,  shade  = .2)
```


Packages used
```{r out.width = '33.33%',fig.show = "hold" }
sessionInfo()
```
